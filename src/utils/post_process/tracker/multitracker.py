...
from ..tracking_utils.ctdet_process import ctdet_post_process
...
class Track(BaseTrack):
    ...
    def multi_predict(stracks):
        ...
        if stracks:
            ...
            multi_cov = np.asarray([st.cov for st in stracks])
            for i, st in enumerate(stracks):
                if st.state != TrackState.tracked:
                    multi_mean[i][7] = 0
            multi_mean, multi_cov = Track.shared_kalman.multi_predict(multi_mean, multi_cov)
    ...
    def activate(self, kalman_filter, frame_id):
        ...
        self.mean, self.cov = self.kalman_filter.initiate(self.tlwh_to_xyah(self._tlwh))
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        self.mean, self.cov = self.kalman_filter.update(
            self.mean, self.cov, self.tlwh_to_xyah(new_track.tlwh)
        )
...
class JDETracker:
    ...
    def __init__(self, conf_thres, track_buffer, max_per_image, num_classes=1, frame_rate=30):
